...
def compartments(rucksacks):
    return [(sack[: len(sack) // 2], sack[len(sack) // 2 :]) for sack in rucksacks]


def groups(rucksacks):
    return [
        [sack for sack in rucksacks[i : i + 3]] for i in range(0, len(rucksacks), 3)
    ]


def duplicates(badges):
    return [
        next(iter(set.intersection(*[set(s) for s in subset]))) for subset in badges
    ]
